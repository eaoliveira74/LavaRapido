// Usa a fetch global fornecida pelo Node 18+
(async ()=>{
  try {
  ...
  console.log('Consulta de geocodificação:', q);
    const gres = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${q}&count=1&language=pt`);
    const gj = await gres.json();
  console.log('Resultado do geocode da Open-Meteo:', (gj.results && gj.results[0]) ? [gj.results[0].latitude, gj.results[0].longitude] : null);
    if ((!gj.results || !gj.results[0])) {
  console.log('Geocodificação da Open-Meteo não retornou resultados — tentando a alternativa Nominatim');
  ...
  console.log('Resultado do Nominatim:', nj && nj[0] ? [nj[0].lat, nj[0].lon] : null);
      if (!(nj && nj[0])) return;
    ...
    console.log('Dias com previsão do tempo:', wj.daily && wj.daily.time, wj.daily && wj.daily.weathercode);
